////
//// Created by Lukas Brand on 12.04.2023.
////
// #include "df_list.h"
//
// Node *newNode(int x) {
//     Node *pnt = (Node *) malloc (sizeof(Node)); /* allocates physical memory for the node */
//     pnt -> value = x;                               /* inserts the information received as input in the field(s) in
//     the list */
...
//     tmp -> next = top;							/* top corresponds to the first element of the list
//     BEFORE this operation,
//												 * which provides to put "tmp" at the
//top of the list,
//												 * thus becoming the new
//"top"
//*/
//     return tmp;									/* returns the new first element of
//     the list
//     */
// }
//
// Node *orderInsert(Node *top, int x) {
//     if (top == NULL || top -> value > x) {
//         /* if top == NULL is true then the element will be positioned at the bottom of the list,
//		 * if top -> value > x then the element will be positioned in the middle of the list.
...
//		 * top -> value > x imposes an increasing order of the elements of the list, but it could well
//		 * be top -> value < x if you wanted to insert elements in descending order. */
//         Node *tmp = newNode(x);						/* creates a node with the information entered
//         as input
//         */
...
//     if (top == NULL)
//         return newNode(x);							/* inserts the item at the bottom of the
//         list
//         */
//     else
...
//     if (top == NULL || top -> value == k)
//         return top;							/* returns the found node (NULL if it was not
//         found)
//         */
//     else
...
//     if (top != NULL) {
//         if (top -> value == k) {	/* if the element was found, it is deleted */
...
//         }
//         else
//             top -> next = deleteNode(top -> next, k);	/* scrolls the list in order to search the item to be
//             deleted
//             */
//     }
//     return top;
